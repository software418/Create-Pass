...
import type { IvisitingArea, VisitingAreaPayload } from '@/master/types';
...
export const useVisitorArea = () => {
  const [visitorArea, setVisitorArea] = useState<IvisitingArea[]>([]);
  ...
  const fetchVisitorArea = useCallback(async () => {
    ...
    try {
      ...
      setVisitorArea(Array.isArray(data) ? data : (data as any).visitingAreas || []);
    } catch (err) {
  ...
  const handleCreate = async (payload: VisitingAreaPayload) => {
    setError(null);
    try {
  ...
  const handleUpdate = async (id: string, payload: Partial<VisitingAreaPayload>) => {
    setError(null);
    try {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      getApiError(err,'Failed to remove Visitor_Area. Reverting table configuration.');
