...
import type { IcarryWith, CarryWithPayload } from '@/master/types';
...
export const useCarryWith = () => {
  const [carryWith, setCarryWith] = useState<IcarryWith[]>([]);
  ...
  const fetchCarryWith = useCallback(async () => {
    ...
    try {
      ...
      setCarryWith(Array.isArray(data) ? data : (data as any).carryWithItems || []);
    } catch (err) {
  ...
  const handleCreate = async (payload: CarryWithPayload) => {
    setError(null);
    try {
  ...
  const handleUpdate = async (id: string, payload: Partial<CarryWithPayload>) => {
    setError(null);
    try {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      getApiError(err,'Failed to remove CarryWith. Reverting table configuration.');
