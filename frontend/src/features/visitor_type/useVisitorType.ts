...
import type { IvisitorType, VisitorTypePayload } from '@/master/types';
...
export const useVisitorType = () => {
  const [visitorType, setVisitortype] = useState<IvisitorType[]>([]);
  ...
  const fetchVisitorType = useCallback(async () => {
    ...
    try {
      ...
      setVisitortype(Array.isArray(data) ? data : (data as any).visitorTypes || []);
    } catch (err) {
  ...
  const handleCreate = async (payload: VisitorTypePayload) => {
    setError(null);
    try {
  ...
  const handleUpdate = async (id: string, payload: Partial<VisitorTypePayload>) => {
    setError(null);
    try {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      getApiError(err,'Failed to remove Visitor_Type. Reverting table configuration.');
