...
import type { Ipurpose, PurposePayload } from '@/master/types';
...
export const usePurpose = () => {
  ...
  const fetchPurpose = useCallback(async () => {
    ...
    try {
      ...
      setPurposes(Array.isArray(data) ? data : (data as any).purposes || []);
    } catch (err) {
  ...
  const handleCreate = async (payload: PurposePayload) => {
    setError(null);
    try {
  ...
  const handleUpdate = async (id: string, payload: Partial<PurposePayload>) => {
    setError(null);
    try {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      getApiError(err,'Failed to remove Purposes. Reverting table configuration.');
